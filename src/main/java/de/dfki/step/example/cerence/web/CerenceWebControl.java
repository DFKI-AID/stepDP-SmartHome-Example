...
import de.dfki.step.example.cerence.SmartHomeDialog;
...
public class CerenceWebControl {
    ...
    public ResponseEntity getVoiceStatus() {
        try {
            ...
            if(SmartHomeDialog.cerence.IsListening())
            ...
            if(body.containsKey(("mic")))
            {
                switch((String)body.get("mic"))
                {
                    case "open":
                        ...
                        SmartHomeDialog.cerence.StartListening();
                        return ResponseEntity.status(HttpStatus.OK).body("ok");

                    case "close":
                        System.out.println("Mix: Close Mic");
                        SmartHomeDialog.cerence.StopListening();
                        return ResponseEntity.status(HttpStatus.OK).body("ok");
    ...
    public ResponseEntity analyzeText(@RequestParam Map<String, Object> body) {
        try {
            if(body.containsKey(("text")))
            {
                ...
                SmartHomeDialog.cerence.AnalyzeText(text);
                return ResponseEntity.status(HttpStatus.OK).body("ok");
            }
            else
            {
